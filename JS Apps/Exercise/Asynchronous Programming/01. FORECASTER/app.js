function attachEvents() {
    ...
    function showUpcomingLocationWeatherInfo(data) {
        ...
        data.forecast.forEach((o) => {
            ...
            let spanSDegrees = createHTMLElement('span', 'forecast-data', degrees);

            let spanSCondition = createHTMLElement('span', 'forecast-data', o.condition);


            divForecast.appendChild(appendChildrenToParent([spanSymbol, spanSDegrees, spanSCondition], spanHolder));
        });
    ...
    function handler(response) {
        if (response.status > 400){
            ...
            throw new Error(`Someting went wrong. Eror: ${response.statusText}`);
        }
